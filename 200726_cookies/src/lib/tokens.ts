...
export const C = {
  ...
  /**
   * Xanh lá: TRẠNG THÁI "hợp lệ / đã xong", cặp với `brand` = "bị từ chối".
   *
   * Đây là hue dành riêng THỨ HAI, và nó không phá luật "màu là danh tính":
   * style_guide vốn đã có màu cho trạng thái (success/fail). Cái mới là nó
   * được cấp một hue riêng thay vì dùng trắng — vì cuống vé xanh/đỏ là quy ước
   * mà ai cũng đọc được ngay, không cần học.
   * Điều kiện: `idColor` phải chừa cả hai dải này ra.
   */
 ...
 *  (brand cam + pass xanh): khoét hai nêm ±GUARD khỏi vòng tròn, rải i lên phần
 *  còn lại; chạm nêm của pass thì nhảy qua. */
