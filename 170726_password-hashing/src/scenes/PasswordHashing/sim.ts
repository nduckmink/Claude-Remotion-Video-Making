...
export type State = {
  ...
  hacker: { x: number; y: number; impact: number; opacity: number } | null;
...
for (let i = 0; i < N_USERS; i++) events.push({ f: A2_THROW + i * 5, kind: "drop", i });
...
const hackerAt = (f: number, attack: number, leave: number, exit: number) => {
  ...
  if (f < hit) {
    ...
    return o <= 0.001 ? null : { x: q.x, y: q.y, impact: 0, opacity: o };
  }
  if (f < leave) {
    // Giữ nguyên chỗ, rung nhẹ theo cú va rồi đứng im mà lục.
    const k = clamp01((f - hit) / 8);
    return { x: HACKER_HIT.x, y: HACKER_HIT.y, impact: 1 - k, opacity: 1 };
  }
  const p = clamp01((f - leave) / exit);
  const q = at(HACKER_HIT, { x: HACKER_REST.x, y: 1760 }, easeIn(p));
  ...
  return opacity <= 0.001 ? null : { x: q.x, y: q.y, impact: 0, opacity };
};
...
const simulate = (): State[] => {
  ...
  for (let f = 0; f <= LOOP; f++) {
    ...
    for (const s of act) {
      ...
      if (thrown) {
        const p = clamp01((f - (A2_THROW + s.i * 5)) / A2_THROW_DUR);
      ...
      if (looted && hacker) {
        ...
        recs.push({
          ...
          y: q.y,
          hashed: s.hashed,
          chars: (s.hashed ? HASH : PASSWORD)[s.i].length,
      ...
      if (s.anvilAt >= 0 && f >= s.anvilAt && f < s.anvilOut) {
        ...
        const struck = since >= SALT_FALL + HAMMER_SWING;
        ...
        recs.push({
          ...
          y: ANVIL.y,
          hashed: struck,
          chars: (struck ? HASH : PASSWORD)[s.i].length,
          // Cú gõ dí bẹp bản ghi xuống rồi nó bật lại — đập thì được, gỡ thì không.
          scale: struck ? 1 - 0.18 * Math.exp(-(since - SALT_FALL - HAMMER_SWING) / 3) : 1,
          opacity: 1,
          rot: 0,
          thrown: false,
        });
