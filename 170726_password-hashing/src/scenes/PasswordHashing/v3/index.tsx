...
/** MÀU LÀ DANH TÍNH: mỗi người dùng một màu, sinh bằng công thức. Ba trạm thì
 *  TRUNG TÍNH — chúng là hạ tầng. Hacker là thứ cam duy nhất động trên stage. */
const USER = Array.from({ length: N_USERS }, (_, i) => idColor(i, N_USERS));

const sfxFile = (e: Ev) =>
  e.kind === "arrive" ? `arrive-${(e.i ?? 0) + 1}.wav` : `${e.kind}.wav`;
...
export const PasswordHashingV3: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: C.bg }}>
      ...
      {/* Ô auth là CÙNG MỘT cánh cửa cho cả hai lần gõ: người dùng gõ vào để
          đăng ký, rồi hacker mang đồ trộm tới gõ lại. Nếu tách làm hai node
          thì mất luôn đối chứng — phải là một. */}
      <Node
        x={AUTH.x}
        y={AUTH.y}
        w={AUTH.w}
